...
def get_opening_hours_py(booking_date):
    ...
    if weekday == 6:  # Воскресенье
        ...
        return time(11, 0), time(
            1, 0
        )  # 01:00 следующего дня — обрабатывается в clean()
    else:  # Пн–Чт
...
class BookingCreateForm(forms.ModelForm):
    ...
    guest_first_name = forms.CharField(max_length=100, required=False, label="Ваше имя")
    guest_last_name = forms.CharField(
        max_length=100, required=False, label="Ваша фамилия"
    )
    ...
    def clean(self):
        ...
        if booking_time < open_time:
            open_str = (  # noqa: F841
                "01:00"
                if close_time == time(1, 0)
                and open_time == time(11, 0)
                and booking_date.weekday() in (4, 5)
                else open_time.strftime("%H:%M")
            )
            raise forms.ValidationError(
                f"Ресторан открывается в {open_time.strftime('%H:%M')}."
            )
        ...
        if booking_date.weekday() in (4, 5):  # Пт/Сб — можно до 01:00 следующего дня
            ...
            if booking_end.date() == booking_date:
                ...
                if booking_end.time() > time(23, 59):
                    raise forms.ValidationError(
                        "В этот день бронирование может завершиться не позже 01:00 следующего дня."
                    )
                ...
                if booking_end.time() > time(1, 0):
                    raise forms.ValidationError(
                        "Бронирование в выходные не может завершиться позже 01:00."
                    )
            else:
                raise forms.ValidationError(
                    "Бронирование не может длиться более суток."
                )
        else:
            # Пн–Чт, Вс — только в тот же день
            if booking_end.date() != booking_date:
                raise forms.ValidationError(
                    "В этот день бронирование должно завершиться в пределах суток."
                )
            if booking_end.time() > close_time:
                raise forms.ValidationError(
                    f"Бронирование не может завершиться позже {close_time.strftime('%H:%M')}."
                )
...
class BookingUpdateForm(BookingCreateForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            print(
                ">>> screenshot_data:",
                screenshot_data[:100] if screenshot_data else "None",
            )
            ...
            if screenshot_data:
                try:
                    ...
                    filename = (
                        f"booking_{booking.pk}_{now().strftime('%Y%m%d_%H%M%S')}.png"
                    )
