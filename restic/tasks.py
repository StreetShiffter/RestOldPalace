...
@shared_task
def expire_started_bookings():
    """Завершает брони, которые начались более 10 минут назад (даже если оплачены)"""
    novosibirsk_tz = ZoneInfo("Asia/Novosibirsk")
    now_local = timezone.localtime(timezone.now(), novosibirsk_tz)
    threshold = now_local - timedelta(minutes=10)

    # Только брони с датой ≤ сегодня и не отменённые
    candidates = Booking.objects.filter(
        is_cancelled=False,
        booking_date__lte=now_local.date(),
    )
    ...
    for booking in candidates:
        ...
        ).replace(tzinfo=novosibirsk_tz)

        if start_local <= threshold:
            booking.is_cancelled = True
            booking.cancelled_at = timezone.now()
            booking.save(update_fields=["is_cancelled", "cancelled_at"])
            # (опционально) отправка сообщения "Спасибо за визит"
